...
class GameRenderer:
    ...
    def tick(self, in_fps: int):
        ...
        while not self._done:
            ...
            self.display_text(f"Счёт: {self._score}                                                                                          Жизни: {self._lives}")

            if self._hero is None:
                self.display_text("Вы проиграли", (self._width / 2 - 256, self._height / 2 - 256), 100)
            if self.get_won():
                self.display_text("Вы выиграли", (self._width / 2 - 256, self._height / 2 - 256), 100)
            pygame.display.flip()
    ...
    def handle_mode_switch(self):
        ...
        print(f"Фаза {str(self._current_phase)}, Разбег, погоня: {str(current_phase_timings)}")
    ...
    def respawn_ghost(self, event_type):
        for ghost in self._ghosts:
            if ghost.respawn_timer == event_type:
                ghost.respawn_timer = None
                self.add_ghost(ghost)
                self._game_objects.append(ghost)
                break
...
class Ghost(MovableObject):
    ...
    def respawn(self, delay: int):
        self.respawn_timer = pygame.USEREVENT + 4
        pygame.time.set_timer(self.respawn_timer, delay)
        self.move_speed = 0.5  
    ...
    def automatic_move(self, in_direction: Direction):
        if in_direction == Direction.UP:
            self.set_position(self.x, self.y - self.move_speed)
        elif in_direction == Direction.DOWN:
            self.set_position(self.x, self.y + self.move_speed)
        elif in_direction == Direction.LEFT:
            self.set_position(self.x - self.move_speed, self.y)
        elif in_direction == Direction.RIGHT:
            self.set_position(self.x + self.move_speed, self.y)
